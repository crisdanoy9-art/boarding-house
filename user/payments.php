...
// Get active tenant with price via beds->rooms
$tenant = $db->prepare("
    SELECT t.id, r.price
    FROM bh.tenants t
...
if ($tenant) {
    $stmt = $db->prepare("
        SELECT *, 
               TO_CHAR(payment_date, 'YYYY-MM-DD HH24:MI:SS') as formatted_date
        FROM bh.payments 
        WHERE tenant_id = ? 
...
<div class="d-flex align-center justify-between mb-4">
    <div>
        <h2 style="font-family:var(--font-display);font-size:1.5rem;color:var(--white);">My Payments</h2>
        <p style="color:var(--muted);font-size:.85rem;">Your payment history and status (real‑time dates)</p>
    </div>
...
<div class="empty-state">
    ...
    <p>Book a room to start your payment history.</p>
    <a href="<?= APP_URL ?>/user/book_room.php" class="btn btn-primary" style="margin-top:16px;">Book a Room</a>
</div>
<?php else: ?>
<div class="stats-grid mb-4" style="grid-template-columns:repeat(auto-fit,minmax(160px,1fr));">
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            <div class="stat-value" style="font-size:1.3rem;"><?= formatCurrency($totalPending) ?></div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            <div class="stat-value" style="font-size:1.3rem;"><?= formatCurrency($totalOverdue) ?></div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            <div class="stat-value" style="font-size:1.3rem;"><?= formatCurrency($tenant['price']) ?></div>
...
<div class="empty-state">
    ...
    <p>Admin will record your payments here.</p>
</div>
<?php else: ?>
<div class="card">
    <div class="card-header">
        <span class="card-title">Payment History</span>
        <button class="btn btn-sm btn-ghost" onclick="exportCSV('pmtTbl','payments.csv')"><i class="fas fa-download"></i> Export</button>
    </div>
    <div class="table-container">
        <table id="pmtTbl">
            <thead>
                <tr><th>Date & Time</th><th>Month</th><th>Amount</th><th>Method</th><th>Reference</th><th>Status</th><th>Receipt</th></tr>
            </thead>
            <tbody>
                <?php foreach($payments as $p): ?>
                <tr>
                    <td style="white-space:nowrap;"><?= formatDate($p['payment_date'], 'M d, Y g:i A') ?></td>
                    ...
                    <td>
                        <?php $bm=['paid'=>'success','pending'=>'warning','overdue'=>'danger']; ?>
